...
public class CarSharingDataConverter implements DataConverter {
	...
	public Object fromMessage(Serializable object) {
		...
		for (ByteString bs : data) {
			try {
				CarSharingStation s = CarSharingStation.parseFrom(bs);
	...
	private GenericPOI extractGenericPOI(CarSharingStation s)
			throws ParseException {
		GenericPOI ge = new GenericPOI();
		ge.setTitle("Car sharing "+s.getId());
		ge.setDescription(createDescription(s));
		ge.setSource("smartplanner-carsharing");
		...
		if (s.hasAvailableCars()) map.put("availableCars", s.getAvailableCars());
		if (s.hasAvailableParkingPlaces()) map.put("availableParkingPlaces", s.getAvailableParkingPlaces());
		if (s.hasRealAvailableCars()) map.put("realAvailableCars", s.getRealAvailableCars());
	...
	private String createDescription(CarSharingStation s) {
		String d = "Car sharing point: "+s.getId()+".";
		if (s.hasAvailableCars()) d += " "+s.getAvailableCars()+" cars available.";
		if (s.hasAvailableParkingPlaces() && s.getAvailableParkingPlaces() > 0) d += " "+s.getAvailableParkingPlaces()+" parking places.";
		return d;
	}


	private POIData createPOIData(CarSharingStation s) {
